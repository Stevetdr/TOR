...
def openRs485Port(portData, rs485):
    logger = Ln.SetLogger(package = __package__)    #ss logger

    for key, val in portData.items():   logger.debug('portData    {0:<15}: {1}'.format(key, val)) #ss logger
    for key, val in rs485.items():      logger.debug('rs485       {0:<15}: {1}'.format(key, val)) #ss logger
    #ss per stampare il contenuto dei dictionary
        # ----------------------------------------------------
        # = RS-485 open/initialize port
        # ---------------------------------------------------- #ss dove prende Rs485 sotto??????
